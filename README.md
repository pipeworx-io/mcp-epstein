...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/epstein_search_documents \
  -H 'Content-Type: application/json' \
  -d '{"query":"non-prosecution agreement"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/epstein_search_documents`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
